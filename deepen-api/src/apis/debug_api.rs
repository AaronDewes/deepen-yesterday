...
/// struct for typed errors of method [`get_user_content_change_to_unread`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUserContentChangeToUnreadError {
...
/// struct for typed errors of method [`reset_user`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_user_content_change_to_unread(
...
) -> Result<(), Error<GetUserContentChangeToUnreadError>> {
    ...
    let uri_str = format!(
        "{}/{locale}/debug/user_content_change_to_unread",
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<GetUserContentChangeToUnreadError> = serde_json::from_str(&content).ok();
...
pub async fn reset_user(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ResetUserError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
